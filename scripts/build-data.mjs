...
const codyChapters={1:[],2:[]}; let current=0;
...
for(let chapter=1;chapter<=50;chapter++){
  ...
  for(const unit of codyChapters[chapter]||[]){
    ...
    chapters[chapter]=chapters[chapter].filter(v=>+v.number<numbers[0]||+v.number>numbers[1]);
...
fs.writeFileSync('dist/data/genesis.json',JSON.stringify({chapters,notes,meta:{source:'OSHB v2.2 / Códice de Leningrado',license:'CC BY 4.0',method:'CODY',hebrewChapters:50,codyChapters:[1,2],verseCount}},null,2));
